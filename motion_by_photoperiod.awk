...
BEGIN {
OFS="\t";
col = 13;   ##### set this to choose which column to process
flag = 0;
motion_light = 0;
...
if((($2 - prevseconds)%300 != 0) && prevdate != $1){   # check for transition between recordings, if so, report and start new day; modulo condition is required to make sure don't start new day at midnight
	if($2>=56160){flag = 1}; # set flag so only report once for today, but only needed if it's after dark in evening.
...
else if($2>=56160 && flag == 0){   # end day at lights out and report
	prevseconds = $2;
	prevdate = $1;
